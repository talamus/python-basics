...
class Noun:
    ...
    def __init__(self, input):                      # define __init__ that is called every time Noun type object is created
        if isinstance(input, Noun):                 # if input is an another Noun
            input = str(input)                      # Start over from a string (easier than to make a deep copy...)
    ...
    def __str__(self, prefix: str=None) -> str:         # Create a new string object from the given object (-> is just a type hint and not a function)
        output = []
        if prefix or self.article:                      # If either prefix or an article is present
            output.append(prefix or self.article)       # Use it
        output += self.adjectives                       # Add adjectives to the list (a += b is shorthand for a = a + b :)
        output.append(self.name)                        # Finally, add the name
        return " ".join(output)                         # Join everything with spaces
